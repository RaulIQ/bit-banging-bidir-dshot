...
void preset_bb_Dshot_buffer_single()
{
    ...
    for (uint8_t i = 0; i < DSHOT_BB_BUFFER_LENGTH - 2; i++)
    {
        ...
        dshot_bb_buffer_1[i * DSHOT_BB_FRAME_SECTIONS] =
            GPIO_BSRR_BR_0 << MOTOR_1;
...
void fill_bb_BDshot_buffer(uint16_t m1_value)
{
    ...
    for (uint8_t i = 0; i < DSHOT_BB_BUFFER_LENGTH - 2; i++)  // последние 2 бита — всегда high
    {
        ...
        {
            // ==== Бит = 1 ====
            // Ничего не делаем — оставляем 0x00, GPIO остаётся LOW до конца DSHOT_BB_1_LENGTH
            dshot_bb_buffer_1[i * DSHOT_BB_FRAME_SECTIONS + DSHOT_BB_0_LENGTH] = 0x00;
        }
        else
        {
            // ==== Бит = 0 ====
